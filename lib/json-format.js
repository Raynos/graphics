...
JSONFormatter.prototype = {
    ...
    decorateWithSpan: function (value, className) {
        return '<span class="' + className + '">' +
            this.htmlEncode(value) + '</span>';
    },

    // Convert a basic JSON datatype (number, string, boolean, null,
    // object, array) into an HTML fragment.
...
function JSONFormat(jsonObj, id) {
    var collapseState = collapseStates[id]

    if (!collapseState) {
        collapseState = collapseStates[id] = {}
    }
    ...
    function addCollapser(item) {
        // This mainly filters out the root object
        // (which shouldn't be collapsible)
        if ( item.nodeName !== 'LI' ) {
            return
        }
    ...
    div.innerHTML = outputDoc

    var items = div.getElementsByClassName('collapsible');
